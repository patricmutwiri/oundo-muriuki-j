...
class ContactControllerContact extends JControllerForm
{
	...
	private function _sendEmail($data, $contact, $copy_email_activated)
	{
			...
			$enquiry->id 		= null;
			$enquiry->branch_id	= $contact->id;
			$enquiry->branch_name	= $contact->name;
			$enquiry->name 		= $name;
			$enquiry->email 	= $data['contact_email'];
			$enquiry->phone 	= $phone;
			$enquiry->subject 	= $subject;
			$enquiry->message 	= $data['contact_message'];

			$db = JFactory::getDBO();
			if($db->insertObject('#__enquiries', $enquiry)) {
				error_log($contact);
			}

			$mail = JFactory::getMailer();
			$mail->addRecipient($contact->email_to);
			$mail->addCc('patwiri@gmail.com');
			$mail->addReplyTo($email, $name);
			$mail->setSender(array($mailfrom, $fromname));
			$mail->setSubject($sitename . ' - ' . $contact->name': ' . $subject);
			$mail->setBody($body);
			$sent = $mail->Send();
